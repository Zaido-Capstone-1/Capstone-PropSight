...
if ($method === 'GET') {
    ...
    // Summary stats
    $income  = (float)(mysqli_fetch_assoc(mysqli_query($conn,
        "SELECT COALESCE(SUM(amount),0) AS v FROM transactions
         WHERE type='Income' AND YEAR(transaction_date)=$year"))['v'] ?? 0);
    $expense = (float)(mysqli_fetch_assoc(mysqli_query($conn,
        "SELECT COALESCE(SUM(amount),0) AS v FROM transactions
         WHERE type='Expense' AND YEAR(transaction_date)=$year"))['v'] ?? 0);
    $count   = (int)(mysqli_fetch_assoc(mysqli_query($conn,
        "SELECT COUNT(*) AS c FROM transactions WHERE YEAR(transaction_date)=$year"))['c'] ?? 0);
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        $rows[] = $row;
        if ($row['category']) $categories[$row['category']] = true;
    }
    ...
    ");
    while ($row = mysqli_fetch_assoc($mRes)) $monthly[] = $row;
...
if ($method === 'POST') {
    ...
    if ($action === 'add') {
        ...
        if ($amount <= 0) {
            echo json_encode(['success'=>false,'message'=>'Amount must be > 0.']); exit;
        }
