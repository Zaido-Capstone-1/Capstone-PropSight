...
if ($method === 'GET') {
    ...
    $res = mysqli_query($conn, "
        SELECT u.user_id, u.first_name, u.last_name, u.email, u.phone,
               u.created_at, u.is_blacklisted, u.is_active,
               COUNT(DISTINCT b.booking_id)                          AS total_stays,
               COALESCE(SUM(b.total_amount),0)                       AS total_spent,
               (SELECT COALESCE(un.unit_name, un.unit_number)
                FROM bookings bx JOIN units un ON un.unit_id=bx.unit_id
                WHERE bx.user_id=u.user_id AND bx.status IN('confirmed','active')
                ORDER BY bx.checkin_date DESC LIMIT 1)               AS current_unit
        FROM users u
    ...
    ");
    $guests = [];
    while ($row = mysqli_fetch_assoc($res)) $guests[] = $row;
...
if ($method === 'POST') {
    ...
    if (!$uid) { echo json_encode(['success'=>false,'message'=>'user_id required.']); exit; }
    ...
    if ($action === 'blacklist') {
        ...
        if (mysqli_query($conn, "UPDATE users SET is_blacklisted=1, is_active=0 WHERE user_id=$uid AND role='user'")) {
            ...
            echo json_encode(['success'=>true,'message'=>'User blacklisted.']);
        } else {
