...
(function () {
    ...
    function _ensureModal() {
      ...
      <div class="rcpt-modal">
        ...
        _overlay.addEventListener('click', (e) => {
            if (e.target === _overlay) closeReceiptModal();
        });
    ...
    function _setStatus(msg) {
        if (!msg) {
            _status.classList.remove('visible');
            return;
        }
    ...
    window.openReceiptModal = async function (bookingId) {
        ...
        try {
            const res = await fetch(_endpointBase() + '?booking_id=' + encodeURIComponent(bookingId) + '&view=1', {
