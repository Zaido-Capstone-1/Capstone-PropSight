...
if (burger && mob) {
    ...
    function closeBookingModal() {
        ...
        if (_bmCurrentStep === 4) {
            ...
            if (isWaiting && window._lastBmBookingData) {
    ...
    function bmSetupPayment() {
        ...
        if (qrAmountEl) qrAmountEl.textContent = amountFmt;
        if (cashAmountEl) cashAmountEl.textContent = amountFmt;
                                ...
                                if (pm.success) {
                                    ...
                                    if (payTab) {
                                        const srcPoller = setInterval(() => {
                                            try {
                                                const tabUrl = payTab.location.href;
                                                const srcMatch = tabUrl.match(/[?&]id=(src_[A-Za-z0-9]+)/);
                                                if (srcMatch) {
