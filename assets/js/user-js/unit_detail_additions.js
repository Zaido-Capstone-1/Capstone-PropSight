(function () {
    ...
    for (let d = new Date(start); d < end; d.setDate(d.getDate() + 1)) {
        ...
        const key = m + '|' + mult;
        if (!buckets[key]) buckets[key] = { month: m, multiplier: mult, nights: 0 };
        buckets[key].nights++;
    }
    ...
    for (const b of Object.values(buckets)) {
        ...
        total          += subtotal;
        rows.push({ month: b.month, multiplier: b.multiplier, nights: b.nights, rate, subtotal });
    }
    return { nights, total: Math.round(total), rows };
}

        const MONTH_SHORT = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
        const SEASON_COLOR = { Peak: '#E74C3C', High: '#deaf37', Low: '#2ECC71' };
        ...
        (function initCardDatePickers() {
            ...
            function updateBreakdown() {
                ...
                const { nights, total, rows } = getSeasonalTotal(ciEl.value, coEl.value, u.priceNum || 0, u.seasonality || {});
        ...
        (function initBookingModal() {
                ...
                if (currentStep === 4 && pendingBid && !window.hasActiveBooking) {
                    fetch(`../../api/user/check_payment_status.php?booking_id=${pendingBid}`)
                        ...
                        .then(st => {
                            ...
                            fetch('../../api/user/cancel_booking.php', { method: 'POST', body: cfd })
                                .then(r => r.json())
                                .then(res => {
            ...
            function _goTo(step) {
                ...
                if (step === 2) {
                    ...
                    const { nights, total, rows } = getSeasonalTotal(ci, co, baseRate, window.UD_UNIT?.seasonality || {}); // ← add rows
                    const nightlyDisplay = nights > 0 ? fmt(Math.round(total / nights)) : fmt(baseRate);
                    const fmtDate = iso => { const d = new Date(iso + 'T00:00:00'); return d.toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }); };
                    ...
                    const color = SEASON_COLOR[label] || '#6b7280';
                    const rvSeason = $('rv-season'), rvSeasonRow = $('rv-season-row');
                    if (rvSeason && label) {
            ...
            window.bmSubmitBooking = window.bmSubmitBooking || function () {
                ...
                co = val('bm-lease');
                const { nights, total: _recomputed } = getSeasonalTotal(
                    ci, co,
                    room.priceNum || 0,
                ...
                fetch('../../api/user/book_unit.php', {
                    ...
                    .then(data => {
                        ...
                        // Call create_paymongo_link.php to get the checkout URL
                        const pmFd = new FormData();
                        pmFd.append('booking_id', bid);
                        pmFd.append('payment_method', method);
                        window.psAppendCsrf?.(pmFd);

                        fetch('../../api/user/create_paymongo_link.php', { method: 'POST', body: pmFd })
                            .then(r => r.json())
                            .then(pmData => {
                            ...
                            fetch(`../../api/user/check_payment_status.php?booking_id=${bid}`)
                                ...
                                .then(st => {
                                    if (st.payment_status === 'paid' || st.booking_status === 'confirmed') {
                                        ...
                                        $('bm-payment-failed').style.display = '';
                                        set('bmFailedRef', `Ref #BK-${String(bid).padStart(4, '0')}`);
                                    } else if (st.payment_status === 'expired') {
                                        clearInterval(window._bmPollInterval);
                                        $('bm-payment-waiting').style.display = 'none';
                                        $('bm-payment-expired').style.display = '';
                                        set('bmExpiredRef', `Ref #BK-${String(bid).padStart(4, '0')}`);
            ...
            function _updateModalSummary() {
                ...
                const { nights, total, rows } = getSeasonalTotal(ci, co, baseRate, window.UD_UNIT?.seasonality || {});
