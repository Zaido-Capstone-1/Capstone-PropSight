...
function handleSetDefault(id, type, btn) {
    ...
    fetch('../../api/user/payment_methods.php', { method: 'POST', body: fd })
        ...
        .then(d => {
            ...
            // Remove all existing default badges and restore "Set Default" buttons
...
// ── EXISTING CARDS (PHP-rendered) ─────────────────
// Wire up Set Default buttons
...
// Wire up Remove buttons for PHP-rendered cards
...
// ── INVOICE DOWNLOAD ───────────────────────────────
document.querySelectorAll('.billing-table .btn-secondary').forEach(btn => {
    btn.addEventListener('click', function () {
...
document.addEventListener('keydown', e => { if (e.key === 'Escape') { closeAddCard(); if (typeof closeSidebar === 'function') closeSidebar(); } });