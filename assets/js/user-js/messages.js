...
function unsentMessage(msgId) {
    ...
    fetch(window.__PS_USER_MSG__.apiUrl, {
        method: 'POST',
        body: (() => { const fd = new FormData(); fd.append('action', 'unsend'); fd.append('message_id', msgId); if (typeof window.psAppendCsrf === 'function') window.psAppendCsrf(fd); return fd; })()
    })
    .then(r => r.json())
    .then(data => {
        if (data.success) {
        ...
        .then(data => {
            ...
            if (data.success) {
                if (tmp) {
                    ...
                    tmp.querySelectorAll('img').forEach(el => el.style.opacity = '1');
                    tmp.querySelectorAll('div[style]').forEach(el => el.style.opacity = '1');
                }
