...
function loadMessages() {
    fetch(`${window.__PS_USER_MSG__.apiUrl}?action=conversation&admin_id=${activeAdminId}`)
        .then(r => r.json())
        .then(data => {
            if (!data.success) { renderChatError(data.message); return; }
...
function renderMessages(msgs, clearFirst = false) {
    ...
    msgs.forEach(m => {
        ...
        const dateStr = d.toLocaleDateString('en-PH', {weekday:'long', month:'long', day:'numeric'});
        ...
        bubble.innerHTML = `
            ...
            ${m.attachment_url ? renderAttachment(m.attachment_url) : ''}
...
function sendMsg() {
    ...
    fetch(window.__PS_USER_MSG__.apiUrl, { method:'POST', body: fd })
        .then(r => r.json())
        .then(data => {
            btn.disabled = false;
    ...
    fetch(`${window.__PS_USER_MSG__.apiUrl}?action=poll&admin_id=${activeAdminId}&since=${since}`)
        ...
        .then(data => {
            ...
            if (data.messages && data.messages.length) {
                // Filter out optimistic bubbles already shown (from_user === self)
                const incoming = data.messages.filter(m => parseInt(m.from_user) !== window.__PS_USER_MSG__.userId);
                if (incoming.length) {
                    renderMessages(incoming, false);
                    lastMsgTs = data.ts || lastMsgTs;
                    updateThreadPreview(activeAdminId, incoming[incoming.length-1].body);
                }
                // Update timestamp even for own messages
                lastMsgTs = data.ts || lastMsgTs;
            }
        })
...
function sendNewMsg() {
    ...
    if (!body) { err.textContent = 'Message cannot be empty.'; err.style.display='block'; return; }
...
window.addEventListener('ps:new_messages', e => {
    (e.detail || []).forEach(m => {
        if (parseInt(m.from_user) === window.__PS_USER_MSG__.userId) return;
        const tid = document.querySelector(`.msg-thread-item[data-admin-id="${m.from_user}"]`);
        if (tid) {
