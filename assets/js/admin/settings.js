...
document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('.policy-tab-btn').forEach(btn => {
        btn.addEventListener('click', function () {
            const key = this.dataset.policyTab;

            document.querySelectorAll('.policy-tab-btn').forEach(b => {
                const active = b === this;
                b.classList.toggle('active', active);
                b.style.color = active ? 'var(--blue-600,#2563eb)' : 'var(--text-soft)';
                b.style.borderBottomColor = active ? 'var(--blue-600,#2563eb)' : 'transparent';
            });

            document.querySelectorAll('.policy-tab-panel').forEach(p => {
                p.style.display = p.dataset.policyPanel === key ? '' : 'none';
            });
        });
    });
});

function savePolicyContent(policyKey) {
    const titleEl = document.getElementById('policy_' + policyKey + '_title');
    const contentEl = document.getElementById('policy_' + policyKey + '_content');
    ...
    fd.append('csrf_token', window.PS_CSRF_TOKEN || '');
    fd.append('policy_key', policyKey);
    fd.append('title', title);
    fd.append('content', content);

    fetch('../../api/settings.php', { method: 'POST', body: fd })
        .then(r => r.json())
        .then(d => showToast(d.message || 'Policy saved.', d.success ? 'success' : 'error'))
        .catch(() => showToast('Network error.', 'error'));
}