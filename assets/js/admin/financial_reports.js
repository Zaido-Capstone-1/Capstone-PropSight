...
function initRevenueMixChart() {
  ...
  if (!revMixCtx || !chartData.revenue_mix || Object.keys(chartData.revenue_mix).length === 0) return;

  const revenueMixLabels = Object.keys(chartData.revenue_mix);
  const revenueMixData = Object.values(chartData.revenue_mix);
  const colors = [blue, grn, gold, '#8B5CF6', '#E74C3C'];
  ...
  revMixChartInstance = new Chart(revMixCtx.getContext('2d'), {
    ...
    data: {
      ...
      datasets: [{
        data: revenueMixData,
        backgroundColor: colors.slice(0, revenueMixLabels.length),
        borderWidth: 3,
        borderColor: '#fff',
        hoverOffset: 0
      }]
...
function initExpenseBreakChart() {
  ...
  if (!expBreakCtx || !chartData.maintenance || chartData.maintenance.length === 0) return;
