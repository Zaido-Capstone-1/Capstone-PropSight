...
const STATUS_CONFIG = {
    occupied: {
        ...
        border: '#86efac',
        label: 'Occupied',
        icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:11px;height:11px;"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>'
    },
    vacant: {
        color: '#2563eb',
        bg: '#dbeafe',
        border: '#93c5fd',
        label: 'Vacant',
        icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:11px;height:11px;"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 21V9"/></svg>'
    },
    maintenance: {
        color: '#dc2626',
        bg: '#fee2e2',
        border: '#fca5a5',
        label: 'Maintenance',
        icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:11px;height:11px;"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg>'
    },
};
...
async function openViewModal(unit) {
    ...
    window.vmGalleryGoTo = function (idx) {
        ...
        if (img) {
            img.style.opacity = '0';
            setTimeout(() => {
                img.src = (window.APP_BASE || '') + '/' + window._vmImages[idx];
                img.style.opacity = '1';
            }, 200);
        }
          ...
          <div style="min-width:0;">
            ...
            <div style="display:flex;align-items:center;gap:8px;margin-top:6px;flex-wrap:wrap;">
              ...
              ${(function(){
                ...
                return '<span style="font-size:11px;font-weight:700;padding:3px 9px;border-radius:99px;background:'+sb.bg+';color:'+sb.color+';">'+sb.lbl+'</span>';
              })()}
            </div>
          </div>
        ...
        <div style="margin-bottom:6px;">
          ...
          ${(status === 'occupied' || status === 'booked') && unit.tenant_name
            ? `<div style="display:flex;align-items:center;gap:12px;padding:12px 14px;background:var(--bg,#f8fafc);border-radius:12px;border:1px solid var(--border,#e2e8f0);">
                ...
                ${status === 'occupied'
                  ? `<span style="margin-left:auto;display:inline-flex;align-items:center;gap:4px;padding:4px 10px;border-radius:99px;background:#dcfce7;border:1px solid #86efac;font-size:11px;font-weight:700;color:#16a34a;letter-spacing:.4px;">
                  ...
                  : `<span style="margin-left:auto;display:inline-flex;align-items:center;gap:4px;padding:4px 10px;border-radius:99px;background:#dbeafe;border:1px solid #93c5fd;font-size:11px;font-weight:700;color:#1d4ed8;letter-spacing:.4px;">
      ...
      `;
        ...
        async function doSave() {
            ...
            try {
                ...
                const data = await fetch('../../api/admin/update_unit.php', {
                    method: 'POST',
                    body: fd
                }).then(r => r.json());
                ...
                if (data.status === 'success') {
                    ...
                    if (card) {
                        ...
                        if (priceEl) priceEl.textContent = '₱' + Number(mergedUnit.rent_amount).toLocaleString('en-US', {
...
function attachViewHandler(btn) {
    btn.addEventListener('click', function () {
        try {
            openViewModal(JSON.parse(this.dataset.unit));
        } catch (e) {
            console.error('View parse error', e);
        }
    });
}
  ...
  `, {
        onMount(bd) {
            ...
            bd.querySelector('#m-save').addEventListener('click', async () => {
                ...
                if (!propertyId) {
                    PS.toast('Please select a property.', 'error');
                    return;
                }
...
function addUnitCard(unit) {
    ...
    card.onmouseenter = () => {
        card.style.boxShadow = '0 8px 32px rgba(0,0,0,.11)';
