...
(function () {
    ...
    function _renderResults(units, dateIn, dateOut, propertyId) {
        ...
        filtered.forEach(u => {
            ...
            cards += `<div class="lp-unit-card" onclick="location.href='pages/user/register.php'"><div class="lp-card-img">${imgTag}<span class="lp-card-type">${_esc((u.unit_type||'UNIT').toUpperCase())}</span><div class="lp-until-pill"><svg viewBox="0 0 24 24" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="3" y1="10" x2="21" y2="10"/></svg>Available<span>${_esc(until)}</span></div></div><div class="lp-card-body"><div class="lp-card-name">${_esc(u.name)}</div><div class="lp-card-loc"><svg viewBox="0 0 24 24"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>${_esc(u.property_name)}${u.city?', '+_esc(u.city):''}</div><div class="lp-card-footer"><div class="lp-card-price">₱${_num(u.rent_amount)} <sub>/ night</sub></div><button class="btn-lp-view" onclick="event.stopPropagation();location.href='pages/user/register.php'">Book Now</button></div></div></div>`;
        });
